...
static int perform_cycle(t_ph *ph)
{
    ...
    // 1) Immediately check if simulation has stopped
    ...
    // 2) Check if THIS philosopher is already dead
...
void *philo(void *arg)
{
    ...
    while (true)
    {
        // Stop if global 'all_dead' or this philo is dead
        pthread_mutex_lock(&ph->sim->mtx_is_dead);
        ...
        // Stop if this philosopher already ate enough times
